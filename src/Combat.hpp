...
public:
    ...
    */
	...
	int combatRunning(	sf::RenderWindow& window,
						std::string IPAddress = "localhost",
						int addressPort = 2715)
	{
	...
	*/
	...
	void setUpServer(std::string IPAddress, int addressPort)
	{
		...
		sf::Time                    pingTime(sf::milliseconds(0)),
                                    pongTime(sf::milliseconds(0)),
                                    timeUntilLaunch(sf::milliseconds(0)),
...
protected:
	...
	*/
	...
	{
		std::vector<CombatEntity> 	*currentTeam(nullptr),
	    ...
	    while (!aboutToStop)
        {
			...
			else
			{
				...
				if (actionType == NONE)
				{
					...
					{
						errorReport("The request sent by the server is handled by the receiving function, but not by serverHandling(). Please contact the developpers", 0);
					}
				}
	...
	*/
	...
	void keyboardInstructions(	std::vector<CombatEntity> *currentTeam,
								std::vector<CombatEntity> *currentEnemies)
	{
		...
		else if (indexes.keyboardMap.isActive("prevPage"))
		{
			...
			case SPELL_CHOOSING:
				...
				break;
			case MAIN:break;
			case CHARACTER_CHOOSING:break;
			case TARGET_CHOOSING:break;
			}
		}
		...
		if (indexes.keyboardMap.isActive("selector"))
		{
			...
			if (indexes.keyboardMap.isActive("charSelector"))
			{
				...
				case TARGET_CHOOSING:
					...
					break;
				case MAIN:break;
				case SPELL_CHOOSING:break;
				case ABILITY_CHOOSING:break;
				}
			}
	...
	*/
	...
	void sendToServer(	CombatEntity& attacker,
						CombatEntity& target,
						AttackType type,
